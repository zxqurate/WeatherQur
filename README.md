...
Современное, полнофункциональное веб-приложение для отображения текущей погоды и прогноза на 5 дней с анимированными эффектами и поддержкой локального сохранения истории поиска.
...
>
> Если сразу после создания ключа появляется ошибка "Ошибка API ключа" — это нормально! Подождите 15-30 минут и повторите попытку.
...
# Запустите локальный сервер (если нужно):
php -S localhost:8000
```
